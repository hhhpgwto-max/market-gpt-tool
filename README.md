...
- 事件时间线的价格反馈从披露/发布时间当天或之后第一个交易日收盘开始计算；公告缺少具体时刻时不会假装知道盘前还是盘后，因此该反馈只是统一口径的历史事实。
- 基金穿透使用基金最新公开披露，并单独返回持仓、行业和资产配置的披露日期。公开接口通常只给前十大持仓，因此组合底层股票覆盖率不会被冒充为完整持仓；未披露部分保留为空。
- 组合暴露默认把输入权重归一化为 100%，也可关闭归一化。基金报告的宽口径行业分布与个股/前十大持仓的细行业标签分开返回，避免把不同分类层级强行相加；重复持仓只基于直持股票和已披露基金持仓。FOF 的基金资产比例会保留为 `fund_pct`，当前不递归猜测下一层基金持仓。
