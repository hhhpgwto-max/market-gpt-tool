...
- `get_a_share_market_snapshot`：在一次有界请求内采集大盘、可选目标证券和最多 9 个同行/指数，批量行情首次超出聚合预算时会在总预算内做一次轻量补偿；统一返回 `snapshot_id`、各组件有效行情时间、最大时间差、来源价差、冲突、推荐来源、缺失字段和数据年龄；不伪造任意历史时刻快照。
