...
- `get_a_share_decision_context`：并发汇总行情、精简分时、四档历史背景、证券状态、相对基准、官方公告、财务和大盘概览，并逐项返回成功、失败、耗时和缺失原因；它是给 GPT 的证据资料包，不替 GPT 判断。
...
- 现有新闻搜索只按股票代码检索，相关性和去重仍不足，因此暂不进入判断资料包。后续新闻升级继续放在本项目的独立组件中，必须先完成“代码+公司名+行业”相关性、可靠来源、去重、发布时间/事件时间区分和原文链接验证，不能与交易所官方公告混为一谈。
