...
- 新股日历使用东方财富公开日历作为时间表数据，并把上交所、深交所、北交所官方规则链接逐项附在结果中。沪深按同市场 T-2 日前 20 个交易日日均非限售 A 股市值计算，最低 1 万元、每 5000 元对应 500 股申购单位；ETF、基金、债券、B 股等不计入。北交所是合格投资者权限加全额资金申购，申购单位为 100 股，不套用沪深市值规则。发行价、上市日尚未公告时列入 `pending_fields`，不会猜测。
