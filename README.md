...
- 分时请求会同时尝试东方财富和腾讯，优先返回最先取得有效交易分钟的一路，总等待预算为 4 秒。
...
- 大盘概览的指数、行业板块和全市场宽度并发获取，总响应预算为 6 秒。慢组件超过预算时优先使用最近一次成功缓存，否则明确返回 `unavailable_within_response_budget`；未完成任务仍可在后台为下一次请求预热。`component_status` 会逐项说明是实时、有效缓存、旧缓存还是本次未取得。
...
- 全市场宽度优先用一次轻量请求取得沪、深、北三市 A 股上涨/下跌/平盘家数与成交额；轻量接口失败时，再并发尝试东方财富和新浪约 5500 只证券的分页快照，避免约 56 页抓取拖慢正常响应。
...
当前以腾讯、新浪和东方财富公开接口作为行情和资讯来源，并在市场总览的行业快照降级路径中保留 `efinance` 计算。免费公开数据源可能延迟、限流或临时不可用；新闻检索可能包含仅提及该股票代码的文章。服务会明确返回数据来源、部分失败、数据年龄、缓存标记和过期状态，以便区分行情时间与服务查询时间。
