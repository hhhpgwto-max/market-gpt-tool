...
- `get_fund_exposure`：查询一个六位基金代码最新公开的前十大持仓、基金报告行业分布、股票/债券/现金/其他资产占比、披露日期和覆盖范围；支持上市 ETF、LOF 与场外公开基金，不把前十大持仓冒充完整仓位。
...
- `get_overnight_risk_packet`：一次返回沪铜、伦铜、美铜、美元兑人民币、纳指期货/指数、富时中国 A50 和恒指期货的最新公开观察及各自时间；美元指数和美国十年期收益率没有已验证免费路线时明确列为缺失，不估算次日涨跌。
...
- 组合暴露默认把输入权重归一化为 100%，也可关闭归一化。基金报告的宽口径行业分布与个股/前十大持仓的细行业标签分开返回，避免把不同分类层级强行相加；重复持仓只基于直持股票和已披露基金持仓。FOF 的基金资产比例会保留为 `fund_pct`，当前不递归猜测下一层基金持仓。组合会复用单基金穿透结果，外层组件固定使用共享有界池和 8 秒预算；直持股票优先采用近期证券资料，冷请求可先用最快公开报价取得名称并明确把行业列为缺失，后台资料成功后由后续请求复用。
