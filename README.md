...
- `get_a_share_decision_context`：并发汇总行情、精简分时、四档历史背景、证券状态、相对基准、官方公告、已筛选新闻、财务和大盘概览，并逐项返回成功、失败、耗时和缺失原因；组件因响应预算未返回时，会给出可直接补查的 `recommended_follow_up_tools`、参数和原因，避免 GPT 把“本次没赶上”误当成“没有数据”。它是给 GPT 的证据资料包，不替 GPT 判断。
